...
**Graphs screen** (`app/(tabs)/graphs.tsx`): renders a line chart with `victory-native`'s `CartesianChart` + `@shopify/react-native-skia`. Metric selection is a custom dropdown `View` (not a native picker); timeframe is a segmented row, since its five options are short and fixed while the metric list is open-ended. The metric list is `Calories`/`Protein`/`Carbs`/`Fat` plus every distinct name in the `exercises` table (reloaded on `useFocusEffect`, since tab screens stay mounted and a mount-only effect would go stale). Queries go through `getNutritionSeries` / `getExerciseVolumeSeries`, which `GROUP BY date` so days with no entries are absent rather than zero — the chart draws those as gaps. The x axis is a **numeric day offset** from the range start with `formatXLabel` rendering it back to a date; a string date `xKey` would space every point evenly and make gaps invisible. The label format follows the range — `Aug 8` up to 30 days, `M/D` to 180, `Aug '25` beyond. The middle ranges stay numeric on purpose: at 3 months five ticks are ~3 weeks apart, so a month-only label can print the same month twice. `xTickCount()` picks a count that divides the day span evenly where one exists (a 7-day range at 5 ticks would step 1.5 days and read `Aug 8, 10, 11, 13, 14`); spans like 29 or 89 are prime, so those fall back to 5 and the rounded gaps differ by at most a day.

**The tick labels only draw because `axisOptions.font` is set.** victory-native renders them as Skia text, and its label branch returns `null` when no font is given rather than falling back to anything — so dropping `font` silently costs every number on both axes while the lines and plot keep drawing normally. `axisFont` comes from Skia's `matchFont()`, which resolves a *system* face, so the project still bundles no `.ttf`. Everything else about the labels — `formatXLabel`, `formatYLabel`, `xTickCount()`, `niceScale()`'s tick count — depends on that one prop being present.

Neither axis carries a title. The caption above the chart names the quantity and its unit, and the x ticks already read as dates, so a title on either would only repeat one of them — a rotated y title down the left cost 54px of plot width to do exactly that, and a bare unit above the tick column was tried and removed too. The caption is also the place for how a value is derived (`sets × reps × weight`), which is too long for an axis title either way.

The chart and its tooltip sit inside a `graphStyle.plot` wrapper, and `chartSize` measures **that wrapper** rather than the card. The tooltip is absolutely positioned, so its origin is the box it sits in, and the wrapper carries no padding or border precisely so its layout box needs no arithmetic to become that origin. Deriving it from the card means reconstructing the same rectangle by subtracting padding and border, which silently desyncs the moment anything is laid out between the card's edge and the chart. The wrapper is also the region a tap dismisses inside.
...
Tapping a point pins a detail window (date, the labeled value, and for exercises a `sets × reps @ weight` line per logged row from `getExerciseEntriesForDate`). It's dismissed by its ✕ or by tapping anywhere else in the plot. Some non-obvious pieces hold that together:
...
- Tap-to-dismiss is a *branch inside the same tap worklet*, not a second gesture: with something pinned it calls `clearSelection` and returns instead of matching. It has to work that way because `handleTouch` matches the nearest point and never misses — there is no empty space in the plot to tap — so dismissing means the tap does nothing else, and moving between points takes two taps. `clearSelection` is therefore defined *above* `tapGesture` (the memo's dep array would otherwise hit the TDZ at render), and whether anything is pinned is mirrored into a `hasSelection` shared value, since the worklet runs on the UI thread and can't read React state. Only the Tap branches; the Pan is untouched, so dragging along the line still moves the pinned point.
- The window claims its own touches with `onStartShouldSetResponder`, or a tap on it would fall through to the gesture detector beneath and dismiss it. RN offers a touch to children first, so the ✕ still wins inside it.

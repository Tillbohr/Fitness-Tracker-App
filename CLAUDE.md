...
Only the y axis is titled, and only with its bare unit — `yAxisUnitFor()` returns `kcal`/`g`/`lbs`/`min`, rendered as a plain RN `Text` above the tick column (Skia text would need a bundled font this project doesn't carry). The x axis has none: its ticks already read as dates. A rotated title down the left cost 54px of plot width to repeat what the caption said, so don't reintroduce one — the caption above the chart is the place for the quantity and for how a value is derived (`sets × reps × weight`), which is too long for an axis title either way.

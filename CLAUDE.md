...
Only the y axis is titled, and only with its bare unit — `yAxisUnitFor()` returns `kcal`/`g`/`lbs`/`min`. It's a plain RN `Text` rather than Skia text, because it sits in the RN layout flow above the chart, not in the canvas. The x axis has none: its ticks already read as dates. A rotated title down the left cost 54px of plot width to repeat what the caption said, so don't reintroduce one — the caption above the chart is the place for the quantity and for how a value is derived (`sets × reps × weight`), which is too long for an axis title either way.

That label is why the chart and its tooltip sit inside a `graphStyle.plot` wrapper, and why `chartSize` measures **that wrapper** rather than the card. The tooltip is absolutely positioned, so its origin is the box it sits in; measuring the card instead would leave every tooltip floating a label-height above the finger. The wrapper carries no padding or border precisely so its layout box needs no arithmetic to become that origin.
